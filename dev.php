<?php
	include_once "includes/header.php";
	include_once "includes/rodape.php";
?>

		<div class="row">
			<div id="corpoLogo">
				<div class="col s12 m6 l4">
					<a href="#modal1" class="modal-trigger"><img id="logo" src="img/logo.png" class="materialboxed"></a> 
					<h4>Butterfly Corporation</h4>
				</div>
				<div class="col s1 m1 push-m1 l1">
					<div id="linha"></div>
				</div>
				<div class="col s6 m5 l3 pull-l1 ">
					<h4>Orientador(a):</h4>
					<h6>Profª Graciete Henriques</h6>

					<h4>Desenvolvimento de Sistemas</h4>
					<h6>1º semestre de 2020</h6>
				</div>
				<div class="col s12 m12 push-m1 l1 pull-l1">
					<div id="enviarTcc">
						<h5 class="center text">Insira ao arquivo de TCC(.docx) aqui</h5>
						<img src="img/icon.png" id="iconeDow" class="img-responsive">
					</div>
				</div>
				<div class="col s12 m12 push-m1 l4 pull-l1">
					<div id="pegarTcc">
						<h5 class="center text">O TCC corrigido aparecerá aqui para download</h5>
						<a href=""><img src="img/icon.png" id="iconeDow2"></a>
					</div>
				</div>
			</div>
		</div>	
		<div class="row">
			<div class="col s12 m12 l4">
				<textarea id="descTcc" placeholder="Insira aqui um breve descritivo sobre sua equipe/projeto de tcc"></textarea>
			</div>
		</div>